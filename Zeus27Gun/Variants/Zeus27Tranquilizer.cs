...
{
    public class Zeus27Tranquilizer : Zeus27Component
    {
        public override uint Id { get; set; } = 51;
        public override string Name { get; set; } = "Zeus27 - Транквилизатор.";
        ...
        public string HighlightSecondColor { get; set; } = "#00ebd3";
        ...
        private new void OnShot(ShotEventArgs ev)
        {
            ...
            ev.Target.EnableEffect(EffectType.Flashed, 40f);
            ev.Target.EnableEffect(EffectType.Ensnared, 45f);
            ev.Target.EnableEffect(EffectType.Deafened, 35f);
        }
        ...
        public IEnumerator HintsHandler(Player player)
        {
            while (!Round.IsEnded && Round.IsStarted && player.IsAlive && Check(player.CurrentItem))
            {
                RueDisplay.Get(player).Show(
                    new Tag(),
                    new BasicElement(110, "<size=25><b><color=#F79100>Вы используете Zeus27 - Транквилизатор!</color></b></size>"), 1.1f);

                foreach (var spec in player.CurrentSpectatingPlayers)
                {
                    RueDisplay.Get(spec).Show(
                        new Tag(),
                        new BasicElement(110, "<size=25><b><color=#F79100>Игрок использует Zeus27 - Транквилизатор!</color></b></size>"), 1.1f);
